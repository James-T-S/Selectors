...
namespace SystemsProgrammingAssesment
{
    internal class Program
    {
        ...
        static int movement(char lastInput)
        {
            ...
            HashSet<(int x, int y)> lastPosistions = new HashSet<(int x, int y)> { (playerX, playerY) };
            ...
            if (lastPosistions.Count() == playerLength) lastPosistions.Add((playerX, playerY));

            for (int i = 0; i < playerLength; i++) gameMap[lastPosistions.Last().y][lastPosistions.Last().x] = '.';

            lastPosistions.Remove(lastPosistions.Last());

            playerY += movementKey[lastInput].y;
            playerX += movementKey[lastInput].x;

            foreach((int x, int y) pos in lastPosistions)
            {
                gameMap[pos.y][pos.x] = player;
            }
            gameMap[playerY][playerX] = player;
            lastPosistions.Add((playerX, playerY));
